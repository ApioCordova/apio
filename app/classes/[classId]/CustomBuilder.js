...
/**
 * Full-screen builder for teacher-authored "custom" assignments.
 * Custom questions are COPIES (pool='custom', custom_assignment_id set) so they
 * never touch admin source content. Deleting the parent cascades the copies.
 */
export default function CustomBuilder({ classId, course, tone = '#00b395', onClose, onSaved }) {
  ...
  const [setItems, setSetItems] = useState([])       // working set, ordered
  const [bank, setBank] = useState([])               // source questions across the course
  const [bankLoading, setBankLoading] = useState(false)
  const [unitFilter, setUnitFilter] = useState('all')
  const [poolFilter, setPoolFilter] = useState('all') // all | lesson | practice
  ...
  // lessonId -> { unitId, unitNumber, unitName, lessonTitle, label }
  ...
  async function loadBank() {
    ...
    if (!lessonIds.length) { setBank([]); setBankLoading(false); return }
    const { data } = await supabase
      .from('questions')
      .select('*')
  ...
  async function deleteCustom(a) {
    ...
    const { error } = await supabase.from('class_assignments').delete().eq('id', a.id) // cascade removes copies
    if (error) { flash('Delete failed: ' + error.message); return }
    flash('Deleted'); await loadList(); onSaved?.()
  }
  ...
  async function save() {
    ...
    if (editId) {
      const { error: upErr } = await supabase.from('class_assignments')
        .update({ title: clean, due_date: fromLocalInput(due), allow_retry: retry })
        .eq('id', editId)
      if (upErr) { setSaving(false); flash('Save failed: ' + upErr.message); return }
      // simplest correct strategy: clear and re-insert the copies
    ...
    await loadList(); onSaved?.(); setView('list')
  }
  ...
  const Overlay = ({ children }) => (
    <div className="fixed inset-0 z-[200] flex items-start justify-center p-3 md:p-6 overflow-y-auto" style={{ background: 'rgba(26,29,41,0.6)' }} onClick={onClose}>
      <div className="w-full max-w-5xl bg-white border-[3px] border-gray-900 rounded-2xl shadow-[8px_8px_0_#1a1d29] my-auto" onClick={(e) => e.stopPropagation()}>
        {children}
      </div>
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[210] px-4 py-2 bg-gray-900 text-white rounded-xl text-sm font-bold shadow-lg">{toast}</div>
      )}
    </div>
  )
  ...
  const DiffBadge = ({ d }) => d && DIFF[d] ? (
    <span className="px-2 py-0.5 rounded-full text-[10px] font-black tracking-wider border border-gray-900 text-white" style={{ background: DIFF[d].color }}>{DIFF[d].label}</span>
  ) : null
    ...
    return (
      <Overlay>
        <div className="flex items-center justify-between gap-3 px-6 py-4 border-b-[3px] border-gray-900">
          <div>
            <p className="text-xs font-mono tracking-widest uppercase" style={{ color: tone }}>// custom assignments</p>
            <h2 className="text-2xl font-black tracking-tight">Build your own</h2>
          </div>
          <button onClick={onClose} className="w-9 h-9 border-2 border-gray-900 rounded-full bg-white flex items-center justify-center font-bold shadow-[2px_2px_0_#1a1d29]" aria-label="Close">✕</button>
        </div>

        <div className="p-6 space-y-4">
          <button onClick={startNew} className="w-full px-5 py-3 text-white border-[2.5px] border-gray-900 rounded-xl font-black uppercase tracking-wide text-sm shadow-[4px_4px_0_#1a1d29]" style={{ background: tone }}>
            + New custom assignment
          </button>

          {loadingList ? (
            <p className="text-gray-500 font-mono text-sm py-6 text-center">Loading…</p>
          ) : customList.length === 0 ? (
            ...
            <div className="space-y-3">
              {customList.map((a) => (
                <div key={a.id} className="flex items-center gap-3 flex-wrap border-[3px] border-gray-900 rounded-2xl p-4 bg-white shadow-[4px_4px_0_#1a1d29]">
                  <span className="w-12 h-12 shrink-0 border-2 border-gray-900 rounded-xl flex items-center justify-center text-xl" style={{ background: `${tone}22` }}>🛠</span>
    ...
    <Overlay>
      ...
      <div className="flex items-center justify-between gap-3 px-6 py-4 border-b-[3px] border-gray-900">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <h2 className="text-xl font-black tracking-tight truncate">{editId ? 'Edit custom assignment' : 'New custom assignment'}</h2>
        </div>
        <button onClick={onClose} className="w-9 h-9 border-2 border-gray-900 rounded-full bg-white flex items-center justify-center font-bold shadow-[2px_2px_0_#1a1d29]" aria-label="Close">✕</button>
      </div>

      {/* meta */}
      <div className="px-6 py-4 border-b-2 border-gray-200 grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <button type="button" onClick={() => setRetry((v) => !v)} className="md:col-span-2 flex items-center gap-3 w-full text-left px-4 py-3 border-2 border-gray-900 rounded-xl bg-white shadow-[3px_3px_0_#1a1d29]">
          <span className="w-11 h-6 rounded-full border-2 border-gray-900 flex-shrink-0 relative" style={{ background: retry ? tone : '#e5e7eb' }}>
      ...
      {/* two panes */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-0 md:gap-4 p-4 md:p-6">
        {/* LEFT — question bank */}
        <div className="border-[2.5px] border-gray-900 rounded-2xl p-4 mb-4 md:mb-0">
          ...
          <div className="space-y-2 max-h-[46vh] overflow-y-auto pr-1">
            {bankLoading ? (
              <p className="text-gray-500 font-mono text-sm py-6 text-center">Loading questions…</p>
            ) : bankFiltered.length === 0 ? (
              <p className="text-gray-500 font-mono text-sm py-6 text-center">No questions match.</p>
            ) : bankFiltered.map((q) => {
              const meta = lessonIndex[q.lesson_id]
              ...
              return (
                <div key={q.id} className="border-2 border-gray-900 rounded-xl p-3 bg-white">
                  ...
                  <p className="text-sm font-medium mb-2 line-clamp-2">{q.stem}</p>
                  <button onClick={() => addFromBank(q)} disabled={added}
                    ...
                    style={added ? { background: '#e5e7eb' } : { background: tone, color: '#fff' }}>
                    {added ? '✓ Added' : '+ Add'}
                  </button>
                </div>
              )
        ...
        {/* RIGHT — working set */}
        <div className="border-[2.5px] border-gray-900 rounded-2xl p-4">
          ...
          <div className="space-y-2 max-h-[46vh] overflow-y-auto pr-1">
            {setItems.length === 0 ? (
              <p className="text-gray-500 font-mono text-sm py-6 text-center">Add questions from the bank, or write your own.</p>
            ) : setItems.map((it, idx) => (
              <div key={it._key} className="border-2 border-gray-900 rounded-xl bg-white">
                <div className="flex items-start gap-2 p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-sm font-medium line-clamp-2">{it.stem}</p>
                  </div>
                  <div className="flex flex-col gap-1 shrink-0">
                    <button onClick={() => setEditingIdx(editingIdx === idx ? null : idx)} className="px-2.5 py-1 border-2 border-gray-900 rounded-lg text-xs font-bold" style={{ background: tone, color: '#fff' }}>{editingIdx === idx ? 'Done' : 'Edit'}</button>
                        ...
                        {it.choices.map((c, ci) => (
                          <div key={ci} className="flex items-center gap-2">
                            <button type="button" onClick={() => patchItem(idx, { answer: ci })} className="w-8 h-8 shrink-0 border-2 border-gray-900 rounded-md text-sm font-black"
                              style={it.answer === ci ? { background: tone, color: '#fff' } : { background: '#fff' }}>{String.fromCharCode(65 + ci)}</button>
      ...
      {/* footer */}
      <div className="flex items-center justify-between gap-3 px-6 py-4 border-t-[3px] border-gray-900">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={save} disabled={saving} className="px-6 py-2.5 text-white border-[2.5px] border-gray-900 rounded-xl font-black uppercase tracking-wide text-sm shadow-[4px_4px_0_#1a1d29] disabled:opacity-50" style={{ background: tone }}>
